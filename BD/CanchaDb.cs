...
namespace Administracion_Torneos.BD
{
    public class CanchaDb
    {
        ...
        public Cancha GetCancha(int Nocancha)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    nCancha.precio_alquiler = reader.GetDecimal(4);
        ...
        {
            string query = $"INSERT INTO CANCHA( Nombre, Capacidad, estatus,precio_alquiler) VALUES ( @Nombre, @Capacidad, @estatus,@precio_alquiler)";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand sql = new SqlCommand(query, connection);
                ...
                sql.Parameters.AddWithValue("@precio_alquiler", nCancha.precio_alquiler);
                try
                {
                    connection.Open();
                    sql.ExecuteNonQuery();
        ...
        public void UpdateCancha(int NoCancha, string Nombre, string Capacidad, string estatus,decimal precio_alquileres)
        {
            string query = "UPDATE CANCHA set Nombre=@Nombre, Capacidad=@Capacidad, estatus=@estatus, precio_alquiler=@precio_alquiler WHERE NoCancha=@NoCancha";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                command.Parameters.AddWithValue("@precio_alquiler", precio_alquileres);
                try
                {
                    connection.Open();
